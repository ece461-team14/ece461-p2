...
if [[ -n "$1" && "$1" == *.postman_collection.json ]]; then
    echo "Running Newman tests on $1..."
    newman run "$1" --reporters cli -e dev-bucket.postman_environment.json --export-environment dev-bucket.postman_environment.json
    ...
    echo "Invalid argument. Please provide a valid *.postman_collection.json file or no argument to run all tests."
